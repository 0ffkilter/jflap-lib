...
public final class IO {
    ...
    public static TuringMachine loadTuring(String location){
        // Check if the location is a file
        File f = new File(location);
        if (f.isFile()){
            return loadAutomaton(f);
        ...
        // Check if it is a valid URL
        try {
            return (TuringMachine)new XMLCodec().decode(new URL(location).openStream());
        } catch (MalformedURLException e) {
            throw new RuntimeException(e);
        } catch (IOException e){
            throw new RuntimeException(e);
    ...
    public static TuringMachine loadTuring(File file){
        if (!file.isFile()) throw new RuntimeException(file.getAbsolutePath() + " does not exist");
        return (TuringMachine)new XMLCodec().decode(file, null);
    }
